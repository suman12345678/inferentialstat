...
```
#mean of simulated_exp
```{r}
means<-apply(simulated_exp,2,mean)
```
#dist mean
```{r}
...
var_theory
```
#check if the dist is approx normal
```{r}
...
```
# compare the distribution of averages of 40 exp to a normal distribution
```{r}
qqnorm(means)
qqline(means, col = 2)
...
Since the p-value is greater than 0.05 and the confidence interval of the test contains zero then supplement types have no impact on Tooth growth.

now compare tooth growth by dose, looking at the different pairs
...
#As the p-value of each test close to zero and the confidence interval of each test does not cross over zero (0).
...
From t-test analysis from above, we can conclude that supplement delivery method has no effect on tooth growth/length, however increased dosages do result in increased tooth length.
